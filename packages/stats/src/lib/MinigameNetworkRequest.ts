...
import { ApiType } from "resources/minigames/minigames.types.ts";
		...
		(async () => {
			try {
				const result = await fetch(
					`${config.endpoint}${props.url}${
						props.type === ApiType.WithParams ? '&' : '?'
					}offset=${props.offset * config.defaultLimit}&amount=${
