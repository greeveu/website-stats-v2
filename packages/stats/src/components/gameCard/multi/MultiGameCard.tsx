 ...
 * @constructor
 */
export const MultiGameCard: React.FunctionComponent<GameCardProps> = (
	props,
) => {
	...
	const [overrideSubTitle, setOverrideSubTitle] = useState<string | null>(
		null,
	);
	...
	return (
		<div className={style.root} onMouseLeave={onMouseLeave}>
			{props.minigames.map((minigame, key) => {
				return (
					<div
						key={key}
						className={
							minigame.highlight ? style.highlighted : style.item
						}
					>
						<Link
