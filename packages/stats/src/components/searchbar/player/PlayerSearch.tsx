...
import {Display} from 'components/searchbar/player/Display';
...
{
	status: Exclude<PlayerSearchStatus, PlayerSearchStatus.Success | PlayerSearchStatus.Invalid>;
}
...
export const PlayerSearch: React.FunctionComponent = observer(() =>
{
	...
	const [state, setState] = useState<PlayerSearchState>({status: PlayerSearchStatus.Waiting});
		...
		(async () =>
		{
			...
			try
			{
				...
				const uuid = await (async () => {
					...
					const result = await fetch(`${config.endpoint}/player/uuid/${searchFor}`);
					if (!result.ok)
					{
				...
				setState({status: PlayerSearchStatus.Success, data: {...playerData, name: name}});
			}
			catch (e: any)
			{
